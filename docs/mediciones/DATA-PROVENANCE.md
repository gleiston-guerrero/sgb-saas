...
| 4 | `tab:res-jacoco` (cobertura JaCoCo vigente) | 08-resultados.tex, §Cobertura | `docs/mediciones/jacoco/report.xml`, `report.csv` y `docs/mediciones/jacoco/README.md`; las carpetas fechadas permanecen como historial y no sustituyen la medición raíz | `jacoco-maven-plugin` vía `./mvnw clean verify` (Maven, no un script propio del repositorio). La cifra defendida se recalcula desde `report.csv`: 1595 líneas cubiertas de 2036 (78,34 %) y 345 ramas cubiertas de 580 (59,48 %). | Reporte raíz vigente: `3fce7e7`. Tabla en el capítulo: `993b5e7`. Canonicidad documentada en esta corrección. |
