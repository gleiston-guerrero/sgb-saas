...
rechaza (spring-projects/spring-data-jpa\#3393). Sin SQL nativo en el
proceso: cero \texttt{nativeQuery = true} y cero \texttt{createNativeQuery}
en \texttt{src/main} (verificado por \texttt{scripts/verify-p5-nativequery.py}).
...
queda en 0 apariciones de \texttt{nativeQuery = true} y 0
\texttt{createNativeQuery} en \texttt{src/main}; la expiración de
reservaciones usa el procedimiento vía el \emph{scheduler}.
...
usa \texttt{createNativeQuery} en ningún punto de \texttt{src/main}: las
rutinas principales van por \texttt{StoredProcedureQuery} posicional y los
reportes por Criteria/JPQL con parámetros bindeados. El valor de cada
parámetro nunca se concatena, formatea ni interpola dentro de la cadena
SQL.
