...
\texttt{@Procedure} o \texttt{@NamedStoredProcedureQuery}. Las anotaciones
\texttt{@Procedure} documentan ese mapeo en los repositorios y existen
\texttt{@NamedStoredProcedureQuery} en las entidades \texttt{Loan} y
\texttt{Fine}; la ejecución real usa \texttt{StoredProcedureQuery} JPA con
binding exclusivamente posicional (ver Listado
\ref{lst:sp-crear-prestamo-java}), porque el proxy estándar de
\texttt{@Procedure} de Hibernate~6 genera sintaxis de parámetros nombrados
de PostgreSQL dentro del escape JDBC \texttt{\{call ...\}} y pgjdbc la
rechaza (spring-projects/spring-data-jpa\#3393). El criterio P5 queda
cerrado con cero \texttt{nativeQuery = true} y cero \texttt{CALL} nativos
en \texttt{*CustomImpl} para procedimientos del dominio (verificado por
\texttt{scripts/verify-p5-nativequery.py}). Existe una sola llamada a
\texttt{createNativeQuery} fuera de repositorios, en \texttt{AuditAspect},
para \texttt{set\_config(...)} de PostgreSQL con parámetro bindeado; no
invoca un procedimiento almacenado del dominio ni concatena entrada.
El siguiente listado muestra el método vigente hoy en
...
filas) se reimplementaron en Criteria/JPQL con cómputo Java de semántica
idéntica (P5): las funciones SQL quedan en BD solo para psql directo y
JPA ya no las invoca. Tras la migración, el conteo estático del backend
queda en 0 apariciones de \texttt{nativeQuery = true} y 0 \texttt{CALL}
nativos en \texttt{*CustomImpl}; se conserva únicamente el
\texttt{createNativeQuery} justificado de \texttt{AuditAspect} para
\texttt{set\_config(...)}. La expiración de reservaciones usa el
procedimiento vía el \emph{scheduler}.
...
las rutinas principales se usa \texttt{StoredProcedureQuery} posicional;
en los reportes, Criteria/JPQL con parámetros bindeados por separado. El
valor de cada parámetro nunca se concatena, formatea ni interpola dentro
de la cadena SQL.

En síntesis: \textbf{la regla A.2.1 prohíbe construir SQL concatenando
entrada de usuario dentro del texto de la consulta}. Este proyecto ya no
usa \texttt{createNativeQuery} para invocar procedimientos del dominio:
las rutinas principales van por \texttt{StoredProcedureQuery} posicional y
los reportes por Criteria/JPQL con parámetros bindeados. La única llamada
de \texttt{AuditAspect} fija \texttt{set\_config(...)} con parámetro
bindeado. El valor de cada parámetro nunca se concatena, formatea ni
interpela dentro de la cadena SQL.
