...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item \textbf{Protocolo de usabilidad SUS}: plantilla de consentimiento
    ...
    (\autoref{sec:tf-sus}). No hay respuestas SUS, consentimientos
    firmados ni figuras derivadas versionadas en esta entrega; el bloque
    permanece en $N=0$.
\end{itemize}
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item No existen formularios SUS firmados para esta entrega, porque la
    corrida queda diferida. Cuando se ejecute, los formularios
    \textbf{no se subirán a este repositorio público bajo ninguna forma};
    se custodiarán, según \texttt{ETHICS.md}, en un medio separado
    (físico o digital con acceso restringido).
\end{itemize}
...
\begin{itemize}[leftmargin=1.2cm]
  \item \textbf{Cajas}: autor del mayor volumen de commits del proyecto
    (727 commits humanos sin merges según
    \texttt{git -c log.mailmap=true shortlog -sne --no-merges HEAD} al
    rev de redacción; conteos vigentes por rol en
  ...
  \item \textbf{Loor}: autor de la mayoría de los commits de
    ...
    estadístico del \autoref{cap:resultados}. 329 commits humanos sin
    merges según la misma consulta con \texttt{.mailmap} (cifra al rev
    de redacción; conteos vigentes por rol en
    \texttt{CONTRIBUCIONES.md}).
  \item \textbf{Panamá}: autor de los componentes de frontend de
    Préstamos, Reservaciones y Multas (\texttt{frontend-angular/}) y de
    varias historias de usuario en \texttt{docs/requisitos/}. 291
    commits humanos sin merges según la misma consulta con
    \texttt{.mailmap} (cifra al rev de redacción; conteos vigentes por
    rol en \texttt{CONTRIBUCIONES.md}). Asignado a la ejecución de las
    pruebas SUS pendientes (OBS-08, \autoref{sec:tf-sus}).
\end{itemize}
