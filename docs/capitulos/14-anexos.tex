...
\autoref{cap:trabajos-relacionados}, el instrumento SUS y su ejecución
anonimizada, el placeholder del pipeline CI/CD pendiente de
...
Este anexo documenta el \textbf{protocolo, cuestionario y resultado}
del instrumento System Usability Scale \citep{brooke1996sus}. La ejecución
cerrada contiene $N=15$ participantes adultos y un export anonimizado;
las identidades y consentimientos originales permanecen privados. La
\autoref{sec:res-usabilidad} reporta media $66{,}00$ e IC95\%
$56{,}95$--$75{,}05$, sin presentar la muestra por conveniencia como
...
\begin{enumerate}[leftmargin=1.2cm]
  ...
  \item \textbf{Puntuación.} La fórmula estándar
    del instrumento produce una puntuación agregada 0--100 por
    participante. En esta ejecución se recalculó cada puntuación desde
    \texttt{docs/mediciones/sus/sus.csv} con
    \texttt{scripts/analyze-sus.py}.
\end{enumerate}
...
agregada se recalcula con la fórmula estándar de Brooke; no se introducen
...
\hline
E3 & Evalúa conceptualmente el artefacto: discute fortalezas, debilidades y limitaciones & \textbf{Cumple} & Cap.\ 12 (Amenazas a la Validez) y Cap.\ 9 (Discusión) discuten debilidades reales (p.\ ej.\ la comparación \texttt{cache\_frío}/\texttt{cache\_caliente} no aísla limpiamente el efecto del caché); los 13 ADR documentan explícitamente los \emph{trade-offs} de cada decisión de arquitectura, no solo la decisión final. \\
\hline
...
\hline
D6 & Evalúa el artefacto en un contexto relevante para la industria (p.\ ej.\ proyectos de código abierto ampliamente usados, programadores profesionales) & \textbf{Parcial} & SGB-SaaS cuenta con una prueba SUS de $N=15$ participantes sobre el despliegue público, pero la muestra es por conveniencia y no demuestra adopción industrial ni representatividad de la población objetivo. \\
\hline
\end{longtable}
\end{center}
...
\hline
La evaluación consiste únicamente en recoger opiniones de usuarios & \textbf{Evitado} & SUS aporta $N=15$ opiniones anonimizadas, complementadas por evaluación técnica (k6, JaCoCo, OWASP y Lighthouse). \\
\hline
...
\hline
REQ-NF-005 & no funcional & Should & N/\allowbreak A - decisión arq. & N/\allowbreak A - decisión arq. & db/\allowbreak schema.\allowbreak sql + db/\allowbreak seed.\allowbreak sql + Flyway (adr-013-estrategia-schema-reproducible.\allowbreak md) & N/\allowbreak A & --- & N/\allowbreak A (esquema/\allowbreak infraestructura, no aplica CRUD-ORM/\allowbreak SP) & --- & implementado \\
\hline
...
\hline
REQ-NF-008 & no funcional & Should & N/\allowbreak A - decisión arq. & N/\allowbreak A - decisión arq. & toda la capa de persistencia (adr-011-gestor-base-datos.\allowbreak md) & N/\allowbreak A & Prestamo\allowbreak Multa\allowbreak Procedure\allowbreak Integration\allowbreak Test (corre contra Postgres real, no contra un mock) & N/\allowbreak A (elección de motor de base de datos) & --- & implementado \\
\hline
REQ-NF-009 & no funcional & Should & N/\allowbreak A - decisión arq. & N/\allowbreak A - decisión arq. & docker-compose.\allowbreak yml + Dockerfiles (adr-007-estrategia-despliegue.\allowbreak md) & N/\allowbreak A & --- & N/\allowbreak A & --- & implementado \\
\hline
...
\textbf{86,30\,\%} de líneas (2337/2708) y \textbf{74,53\,\%} de ramas
(594/797), medida con JaCoCo sobre el corte de cierre
documentado en \autoref{sec:res-cobertura} y con el cierre de ramas de
servicio declarado en \autoref{sec:tf-service-branch-coverage}. Los reportes históricos se
...
El bloque de usabilidad se declara con \textbf{$N=15$}. Se conserva un
export de respuestas anonimizado, sus figuras derivadas y la estadística
recalculable; las identidades y consentimientos originales no se publican.
La autorización adicional para publicar las filas anonimizadas se documenta
como confirmación oral grupal, no como una segunda firma escrita.

Las figuras evaluables del informe quedan rotuladas en inglés: los tres
diagramas C4, el DER, el gráfico de rendimiento p95 y las dos figuras SUS
usan labels/captions en inglés.

La documentación Javadoc del backend queda respaldada por una auditoría
reproducible versionada en
\texttt{docs/mediciones/backend/javadoc-audit.md}. El comando
\texttt{python scripts\textbackslash audit-javadocs.py} cuenta
\textbf{403 de 403} métodos públicos documentados
(\textbf{100,00\,\%}), junto con 679 etiquetas \texttt{@param}, 369
\texttt{@return} y 80 \texttt{@throws}; además,
\texttt{mvn javadoc:javadoc} termina sin errores sobre el backend, como
exige la rúbrica para el nivel Completo.

Los nombres del código del backend quedan respaldados por una auditoría
reproducible versionada en
\texttt{docs/mediciones/backend/english-names-audit.md}. El comando
\texttt{python scripts\textbackslash audit-english-names.py} reporta
\textbf{0 de 283} tipos y \textbf{0 de 642} métodos con raíz española
(\textbf{0,00\,\%}), y una verificación ampliada con un diccionario de
más de 700 voces deja solo 7 métodos con cognados válidos en inglés
(\texttt{error}, \texttt{local}, \texttt{via}, \texttt{principal}),
documentados como excepciones en la propia evidencia. Los identificadores
con raíz española real se renombraron sin tocar contratos externos
(rutas, nombres de parámetros \texttt{@RequestParam} ni firmas SQL).
...
\subsection*{Alcance de corrección de este cierre}

Sí se ajustó el mecanismo de persistencia donde era estable hacerlo: se
retiraron dos variantes \texttt{@Query(nativeQuery = true)} obsoletas de
\texttt{ReservationProcedureRepository}, porque el código de producción y
sus pruebas ya usaban el método \texttt{@Procedure} equivalente. No se
reescribieron las funciones tabulares \texttt{RETURNS TABLE} a
\texttt{REF\_CURSOR}, porque eso cambiaría el contrato SQL público de los
reportes y empeoraría la inspección directa con \texttt{SELECT * FROM
fn\_...(...)}. La cobertura de backend tampoco se modificó en este bloque:
las brechas que requieren nuevos tests de aplicación se conservan como
trabajo de implementación, no como afirmación documental.
