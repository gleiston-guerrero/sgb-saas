...
\textbf{Interpretación por capa (vigente al cierre, \autoref{tab:res-jacoco-capas-vigente}).} La medición vigente deja \texttt{controller} por encima del umbral interno (97,03\,\% de líneas; 75,51\,\% de ramas) y \texttt{service} cruza el umbral en líneas (87,38\,\%) y ramas (77,70\,\%). El detalle numérico queda en el XML versionado para evitar mezclar cifras de cierre con cifras históricas dentro del texto defendido. La \autoref{fig:res-jacoco-paquetes} desagrega la cobertura de instrucciones por paquete desde el CSV canónico.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{mediciones/figuras/fig-jacoco-paquetes.pdf}
\caption{JaCoCo instruction coverage by package (top 12), from the
canonical \texttt{docs/mediciones/jacoco/report.csv} (49 instrumented
