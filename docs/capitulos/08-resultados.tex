...
\textbf{Estadística descriptiva (5 corridas agregadas, serie 2026-09-17).}
La serie anterior (julio/agosto 2026, \texttt{REPORT.md}) queda como
historia metodológica; los números de abajo son la re-medición exigida
al cierre, ejecutada el 2026-09-17 contra el \emph{stack} local
reconstruido desde el commit \texttt{fcda4808} (imágenes
\texttt{sgb-saas-backend/frontend} recompiladas ese día, 50~VUs,
mismo perfil de \texttt{k6/opts.js}). Resumen por corrida y SHA-256 de
los NDJSON en \texttt{docs/mediciones/perf/REPORT.md}.
...
\midrule
cache\_caliente & 9\,821 & 29,84 & 22,63 & 29,86 & [29,25; 30,43] & 46,99 & \textbf{65,60} & 129,36 \\
cache\_frio     & 10\,006 & 11,01 & 10,30 & 3,41 & [10,95; 11,08] & 15,06 & \textbf{17,13} & 23,24 \\
\bottomrule
\end{tabular}
\caption{Estadística descriptiva de \texttt{http\_req\_duration} (ms) por escenario, 5 corridas agregadas (serie 2026-09-17)}
\label{tab:res-perf-descriptivo}
\end{table}

Ambos umbrales exigidos se cumplen con margen amplio: p95
$65{,}60$\,ms $<200$\,ms (caliente), p95 $17{,}13$\,ms $<500$\,ms (frío)
(ver \autoref{tab:res-perf-descriptivo}).
Tasa de error HTTP $\geq$500: $0{,}00$\,\% (0 de 19\,827 peticiones
totales). La distribución completa y el \emph{throughput} por corrida
se visualizan en las \autoref{fig:res-perf-distribucion} y
\autoref{fig:res-perf-throughput}.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{mediciones/figuras/fig-k6-distribucion-latencia.pdf}
\caption{Latency distribution of \texttt{http\_req\_duration} per scenario,
5 runs pooled (2026-09-17 series). Generated from the versioned k6 NDJSON
raws with \texttt{scripts/generar-figuras-evaluacion.py}.}
\label{fig:res-perf-distribucion}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{mediciones/figuras/fig-k6-throughput.pdf}
\caption{Throughput (req/s) per run and scenario, \texttt{GET
/api/v1/libros} under 50~VUs (2026-09-17 series). Generated from the
versioned k6 NDJSON raws with
\texttt{scripts/generar-figuras-evaluacion.py}.}
\label{fig:res-perf-throughput}
\end{figure}
...
Cliff's delta $=-1{,}00$ (efecto \textbf{grande}, convención
$|\delta|\geq0{,}474$; dirección perfectamente consistente en las 5
corridas).
...
\caption{p95 \texttt{http\_req\_duration} per run, \texttt{warm\_cache}
vs.\ \texttt{cold\_cache}, with 95\% CI error bars (bootstrap, 2000
replicates, seed~42). Colorblind-safe Okabe-Ito palette.}
...
(ver \autoref{fig:res-zap-riesgos}).

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{mediciones/figuras/fig-zap-riesgos.pdf}
\caption{ZAP Ajax Spider alerts by risk level (local stack, 2026-08-17,
11 alerts). Source:
\texttt{docs/mediciones/sec/zap/2026-08-17-zap-ajax-full-report.json};
generated with \texttt{scripts/generar-figuras-evaluacion.py}.}
\label{fig:res-zap-riesgos}
\end{figure}
...
\texttt{jacoco-maven-plugin}. La cifra de cierre vigente se calcula
desde los artefactos raíz versionados en \texttt{docs/mediciones/jacoco/}:
\texttt{report.csv}, \texttt{report.xml} y el HTML asociado. Esos
artefactos declaran \textbf{86,30\,\%} de líneas (2337/2708) y
\textbf{74,53\,\%} de ramas (594/797). Esta es la única cifra vigente de
cobertura citada en el entregable final. Las mediciones anteriores quedan
archivadas como evidencia histórica en \texttt{docs/mediciones/jacoco/},
pero no se defienden como resultado de cierre.
...
\texttt{report.csv} son la fuente canónica vigente descrita arriba. Los
cortes anteriores permanecen en subdirectorios fechados como trazabilidad
histórica, pero esta sección solo usa los artefactos raíz; la regla de
canonicidad queda declarada en \texttt{docs/mediciones/jacoco/README.md}.

\textbf{Estadística descriptiva.} Esta métrica es una medición agregada
sobre la totalidad del código analizado (54 clases en la corrida
...
\midrule
Lines & 2337/2708 & \textbf{86,30\,\%} & $\geq$70\,\% \textbf{Sí} \\
Branches & 594/797 & \textbf{74,53\,\%} & $\geq$70\,\% \textbf{Sí} \\
\bottomrule
\end{tabular}
\caption{Cobertura JaCoCo vigente al cierre de la Entrega Final.}
...
analizables cerró en 2708 líneas tras las exclusiones JaCoCo documentadas, con los commits
posteriores al merge de los 8 módulos (nuevos controladores, chatbot,
auditoría, QR, backups, sugerencias de adquisición, etc.), los
procedimientos migrados a mecanismos JPA sin SQL nativo (P5, ver
\texttt{docs/basedatos/P5-MATRIZ-33.md}) y la política de cookie
productiva, y la
cobertura global de líneas supera el \textbf{umbral informal de 70\,\%}
fijado por el equipo para la Entrega Final (86,30\,\% al cierre). Las
capas \texttt{controller} (96,54\,\% líneas, 75,51\,\% ramas),
\texttt{security}, \texttt{scheduling} y \texttt{service} (87,55\,\%
líneas, 77,31\,\% ramas) quedan por encima del umbral interno en líneas
y ramas; las ramas globales cierran en 74,53\,\% y cumplen el umbral de
70\,\% exigido por la rúbrica para servicios y controladores.
...
de cobertura vigente, resumida en \autoref{tab:res-jacoco}.
...
\bottomrule
\end{tabular}
\caption{Cobertura JaCoCo por capa arquitectónica, medición vigente al cierre}
\label{tab:res-jacoco-capas-vigente}
\end{table}

\textbf{Interpretación por capa (vigente al cierre, \autoref{tab:res-jacoco-capas-vigente}).} La medición vigente deja \texttt{controller} por encima del umbral interno (96,54\,\% de líneas; 75,51\,\% de ramas) y \texttt{service} cruza el umbral en líneas (87,55\,\%) y ramas (77,31\,\%). El detalle numérico queda en el XML versionado para evitar mezclar cifras de cierre con cifras históricas dentro del texto defendido. La \autoref{fig:res-jacoco-paquetes} desagrega la cobertura de instrucciones por clase (top 12) desde el CSV canónico.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{mediciones/figuras/fig-jacoco-paquetes.pdf}
\caption{JaCoCo instruction coverage by class (top 12), from the
canonical \texttt{docs/mediciones/jacoco/report.csv} (54 instrumented
classes). Generated with
\texttt{scripts/generar-figuras-evaluacion.py}.}
\label{fig:res-jacoco-paquetes}
\end{figure}
...
Como resume \autoref{tab:res-lighthouse-perfiles}, las 4 categorías cumplen su umbral en \textbf{ambos perfiles} (móvil y escritorio), en las 6 corridas individuales (3 por perfil). Escritorio alcanza 100,0 en todas las categorías (DT 0,0); móvil alcanza 100,0 en Accessibility, Best Practices y SEO, y 86,3 $\pm$ 1,5 en Performance (margen cómodo sobre 80). Se cumple con margen el requisito de la Guía: ``al menos tres corridas por perfil (mobile, desktop) con media y desviación típica por categoría'' (P4/B.10). La \autoref{fig:res-lighthouse-medias} visualiza esas medias post-fix.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{mediciones/figuras/fig-lighthouse-puntajes.pdf}
\caption{Mean Lighthouse scores per category and profile, post-fix
production round (2026-08-18, 3 runs per profile). Computed from the
versioned \texttt{lhci-*-prod-20260818-*.json} raws with
\texttt{scripts/generar-figuras-evaluacion.py}.}
\label{fig:res-lighthouse-medias}
\end{figure}
...
\textbf{Datos crudos.} La corrida del 19 de septiembre de 2026 contiene
$N=15$ respuestas anónimas en
\texttt{docs/mediciones/sus/sus.csv}. El instrumento aplicado, el
manifiesto de consentimientos y las reglas de acceso están versionados en
la misma carpeta. La hoja original con nombres, correos y firmas queda
bajo custodia privada; el manifiesto vincula cada código P01--P15 con su
registro fuente sin publicar PII. La fuente tuvo 17 envíos: se excluyeron
antes del análisis una persona que indicó no poder participar por edad y
un registro con correo de demostración/no verificable. Ninguna exclusión
dependió del puntaje.

\textbf{Estadística descriptiva.} La puntuación Brooke recalculada da
media 66,00, mediana 67,50, DT 16,33, mínimo 45,00 y máximo 95,00; el
IC~95\,\% con $t$ de Student ($df=14$) es [56,95; 75,05]. Trece personas
reportaron completar las tareas y dos las completaron parcialmente. La
muestra es por conveniencia, por lo que describe esta corrida y no permite
generalizar a una población. El valor no se presenta como una etiqueta de
aceptabilidad: queda por debajo de la referencia descriptiva de 68 y debe
interpretarse junto con la dispersión y los comentarios anonimizados.
...
\midrule
Media Likert & 3,73 & 2,60 & 4,13 & 2,60 & 3,80 & 2,60 & 4,20 & 2,80 & 3,60 & 2,47 \\
\bottomrule
\end{tabular}
\caption{Mean raw Likert response by SUS item; negative items retain their original direction.}
\label{tab:res-sus-items}
\end{table}

Como muestran \autoref{fig:res-sus-scores} y
\autoref{fig:res-sus-items}, los resultados varían de forma sustancial
entre participantes. Los textos libres señalan como temas de mejora la
lentitud de carga, la visibilidad del inicio/cierre de sesión y la
búsqueda del catálogo; son observaciones cualitativas, no incidencias
reproducidas por el equipo.

\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{mediciones/sus/sus-score-boxplot.pdf}
\caption{Distribution of individual SUS scores from 15 anonymized participants.}
\label{fig:res-sus-scores}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{mediciones/sus/sus-item-means.pdf}
\caption{Mean response by SUS item; error bars show sample standard deviation.}
\label{fig:res-sus-items}
\end{figure}

\textbf{Estado real.} \textbf{Completo como evidencia P3.} El
instrumento de Brooke, las 15 respuestas anónimas, el manifiesto de
custodia por código y el recálculo están disponibles y son comprobados por
\texttt{scripts/verify-p3-sus.py}. La custodia privada de los
consentimientos protege la identidad sin impedir una revisión docente.
...
3. Cobertura (JaCoCo) & Medición vigente de cierre: 86,30\,\% de líneas (2337/2708) y 74,53\,\% de ramas (594/797), respaldada por el XML versionado; cumple el objetivo interno de 70\,\% en líneas y ramas, con servicios (87,55\,\%/77,31\,\%) y controladores (96,54\,\%/75,51\,\%) sobre el umbral & \textbf{Completo} & Sí \\
...
5. Usabilidad (SUS) & $N=15$ anónimo; instrumento, manifiesto de custodia y recálculo Brooke versionados; media 66,00 e IC~95\,\% [56,95; 75,05] & \textbf{Completo} & Sí \\
\bottomrule
\end{tabularx}
...
Los cinco bloques exigidos por la guía cuentan con evidencia versionada.
En usabilidad, la evidencia corresponde a una sola muestra de conveniencia
y conserva sus limitaciones metodológicas; su cierre documental no implica
que las oportunidades de mejora observadas se hayan corregido ya.
