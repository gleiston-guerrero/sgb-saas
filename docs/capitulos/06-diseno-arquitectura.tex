...
\caption{C4 model, Level~1 (System Context): the five system actors and
their interactions with SGB-SaaS as a black box. Exported automatically
from \texttt{workspace.dsl}.}
...
\caption{C4 model, Level~2 (Containers): Angular frontend, Spring Boot
backend, PostgreSQL, and Redis, aligned 1:1 with
\texttt{docker-compose.yml}. Exported automatically from
\texttt{workspace.dsl}.}
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item \textbf{PostgreSQL~16} -- usuarios, roles/permisos, catálogo,
    préstamos, reservas, multas y bitácora de auditoría (45 tablas).
  \item \textbf{Redis~7} -- blacklist de tokens JWT revocados (logout /
    revocación) y caché del listado de libros con TTL configurable
    externamente.
  \end{itemize}

El diagrama entidad-relación del esquema PostgreSQL
(\autoref{fig:der-real}) combina dos artefactos complementarios. El PDF
técnico \texttt{docs/diagramas/der-real.pdf} muestra las 45 tablas con
sus columnas, tipos, nulidad y cardinalidades con identificadores
reales de base de datos. Por legibilidad del documento final y para
cumplir la rúbrica de figuras en inglés, la figura insertada
(\texttt{docs/diagramas/er-english.pdf}) es una vista de resumen por
áreas en inglés: cada área lista sus tablas reales con columnas clave,
marcas \texttt{PK}/\texttt{FK} y cardinalidades en las relaciones, y se
genera con \texttt{scripts/generate-english-er-figure.py}, cuya opción
\texttt{--check} verifica la especificación contra
\texttt{information\_schema} (86 columnas en 30 tablas). El archivo histórico
...
\caption{Entity relationship overview with English area labels: real
tables and key columns with PK/FK marks and cardinalities, generated
from a spec verified against the PostgreSQL schema. Full table-level
detail with real identifiers in \texttt{docs/diagramas/der-real.pdf}.}
\label{fig:der-real}
\end{figure}
...
\caption{C4 model, Level~3 (Components) for the Spring Boot backend --
21 components grouped by domain (API layer, service layer, security, and
external integration), with relationships verified against the real
codebase. PDF conversion from the versioned source SVG at
\texttt{docs/arquitectura/c4-nivel3-componentes.svg}.}
...
La \autoref{fig:diseno-endpoints-roles} cuantifica el otro eje de la
estrategia híbrida: el control de acceso por rol sobre la API. Cada
barra cuenta las anotaciones \texttt{@PreAuthorize} de método que
mencionan al rol (79 anotaciones en los controladores del backend):
\texttt{GERENTE} aparece en las 79, \texttt{ADMIN} en 75,
\texttt{BIBLIOTECARIO} en 37 y \texttt{LECTOR} en 12 -- el rol gestor
concentra la superficie operativa, con administración plena reservada
al rol \texttt{ADMIN}.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{mediciones/figuras/fig-endpoints-roles.pdf}
\caption{Backend API authorization annotations per role (79 method-level
\texttt{@PreAuthorize} annotations). Counted from the versioned
controllers with \texttt{scripts/generar-figuras-evaluacion.py}.}
\label{fig:diseno-endpoints-roles}
\end{figure}

La \autoref{fig:diseno-migraciones} muestra la evolución del esquema
versionado: migraciones Flyway acumuladas en el tiempo (ADR-013). Cada
punto es la fecha de incorporación de un archivo \texttt{V*.sql} en
\texttt{database/migrations/}, trazable con \texttt{git log}.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{mediciones/figuras/fig-migraciones-acumuladas.pdf}
\caption{Cumulative versioned Flyway migrations over time
(\texttt{database/migrations/V*.sql}). Generated from \texttt{git log}
with \texttt{scripts/generar-figuras-evaluacion.py}.}
\label{fig:diseno-migraciones}
\end{figure}
