...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw=verdeuteq, thick, rounded corners,
      fill=grisclaro, text width=7.0cm, align=center, minimum height=1.2cm,
      font=\small},
    excl/.style={rectangle, draw=gapcolor, thick, rounded corners,
      fill=white, text width=5.6cm, align=left, minimum height=1.2cm,
      font=\footnotesize},
    arr/.style={-{Latex[length=2.2mm]}, thick, draw=verdeuteq}
  ]
  \node[box] (identificados)
    {\textbf{Identified}\\[2pt] Group~A: 15 records\\
     (13 from prior team curation + 2 from targeted
     Crossref/IEEE~Xplore search for this task)};
  \node[box, below=of identificados] (duplicados)
    {\textbf{After removing duplicates}\\[2pt] 15 records\\
     (0 duplicates found)};
  \node[box, below=of duplicados] (cribados)
    {\textbf{Screened by title/abstract}\\[2pt] 15 assessed\\
     0 excluded at this stage};
  \node[excl, right=of cribados] (nota1)
    {The 15 records had already been pre-filtered for topical
     relevance to SGB-SaaS before entering the pool -- hence
     title/abstract screening excludes nothing further here.};
  \node[box, below=of cribados] (textocompleto)
    {\textbf{Assessed in full text / extended abstract}\\[2pt]
     15 assessed};
  \node[excl, right=of textocompleto] (nota2)
    {5 excluded from the primary comparison table: reviews or
     position papers synthesising the state of the art
     (\citet{ayinde2026ai, liabor2023cataloging, adetayo2023chatgpt,
     yan2023chatbotsslr, ayemowa2024genai}), not primary systems
     directly comparable row by row. Kept cited in
     Section~\ref{sec:panorama-relacionados} as context.};
  \node[box, below=of textocompleto, fill=verdelight!25] (incluidos)
    {\textbf{Included in the primary-systems comparison table}\\[2pt]
     \textbf{10 records} (Table~\ref{tab:trabajos-comparativa})};
...
\end{tikzpicture}
\caption{Selection flow adapted from PRISMA~2020 for Group~A
  (library domain and comparable data-access architecture).
  Group~B (15 methodological support references, \autoref{sec:proceso-seleccion})
  is not subject to this funnel.}
...
Préstamo\allowbreak/devolución con QR en biblioteca escolar privada (Surakarta,
Indonesia) &
...
configuraciones ORM &
No evalúa explícitamente una arquitectura híbrida ORM+\allowbreak procedimientos
