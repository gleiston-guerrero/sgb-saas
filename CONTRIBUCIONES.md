...
| P3 SUS (1,4) | **Cerrado con evidencia limitada: $N=15$, media 66,00, muestra por conveniencia** | Panamá Murillo (recolección, anonimización, análisis y verificador); retiro histórico: Cajas Ibarra + Panamá Murillo | Export anonimizado, instrumento, manifest privado de consentimiento, estadísticas y figuras recalculables; la autorización de publicación de filas se documenta como confirmación oral grupal, no segunda firma escrita | `docs/mediciones/sus/`, `scripts/analyze-sus.py`, `scripts/verify-p3-sus.py` | Retiros históricos: `e6df3ff6ad3796f7ea4d41b97674d0741862043d` (Cajas), `1c28085ffcc66098e558b49634397d6984f4df4b` (Panamá); cierre actual: commit de esta rama | `python scripts/verify-p3-sus.py` | *(la aceptación histórica de Cajas/Loor no atribuye esta nueva corrida)* |
...
| EV-1/EV-2 Expediente+verify | Operativo (solo-lectura, UTF-8 sin PYTHONUTF8, exit 0 sin FALLO; P3 validado con datos anonimizados; P10 requiere Docker solo en ejecución local) | Panamá Murillo | Expediente sin fechas futuras ni elipsis; orquestador `verify-all.py`; verificadores P1-P12; evidencia por SHA; job CI | `VERIFICACION.md`, `Makefile`, `scripts/verify-all.py`, `scripts/verify-*.py`, `docs/evidencia/examen/` | Historial previo y cierre P3 actual en esta rama | `python scripts/verify-all.py` | *(vacía)* |
...
fila: P1 Panamá+Cajas; P2 Loor+Panamá; P3 Panamá como ejecución SUS
actual y Cajas+Panamá solo como retiro histórico; P4 Panamá+Loor; P5 Panamá como migración final
