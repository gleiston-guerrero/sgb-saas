...
Rev de referencia: `fcda4808` (rama `fix/rescate-produccion-examen`,
padre del commit de evidencia de este expediente). El commit de
evidencia suma +1 a Panamá (solo docs de examen); re-sincronizar al
...
git rev-list --count --no-merges <rev>                            # 1425
git log --no-merges --author=<icajasi|mloorm14|mpanamam> --format=%h -- <ruta>  # por área
```

Totales globales a `fcda4808`: **Cajas 763, Loor 343, Panamá 317**
...
| Rol CRediT | Cajas (763) | Loor (343) | Panamá (317) |
|---|---|---|---|
...
| Data curation | **10** `mediciones/sec` (índice, commit `4b7f50a`) + **5** `mediciones/sus` (esqueleto + script, `1ad718b`) | **21** `mediciones/sec` + **8** `mediciones/perf` + `DATA-DICTIONARY.md`, `DATA-PROVENANCE.md` | **2** `mediciones/sus` (retiro declarado N=0, OBS-08) + `ETHICS.md`, `DATA-DICTIONARY.md` (sección SUS, 22 campos) |
| Formal analysis | — | **5** `k6/`+`perf-analysis.py` (Wilcoxon pareado + Cliff's delta) + **8** `mediciones/perf` | `scripts/sus-analysis.ipynb` (reescritura: descriptiva, IC95%, desglose por ítem) + **4** `scripts` |
| Funding acquisition | No aplica | No aplica | No aplica (PFC sin financiamiento externo) |
| Investigation | **10** `mediciones/sec` + **76** `src/test` + reporte SQL A.2.3 (`b683847`) | **21** `mediciones/sec` + **6** `bibliografia.bib` (verificación Crossref) | **30** `src/test` + **8** `docs/requisitos` (E2E frontend préstamos/reservas/multas, `84ffc30`) |
...
| Validation | **76** `src/test` + **10** `mediciones/sec` (ZAP, E2E préstamo-devolución-multa, SP multi-OUT `baa0820`) | **21** `mediciones/sec` + **8** `lighthouse` + **15** `jacoco` (verificación en vivo por cambio) | **30** `src/test` + E2E frontend vs backend real + WCAG AA (`1640dd9`, `b1399a2`) |
...
| Writing – review & editing | **12** `CITATION.cff` + **11** `docs/trazabilidad` (unificación cifras, refs rotas) | **57** `docs/capitulos` + **6** `bibliografia.bib` (Peffers, Hevner) + **15** `CITATION.cff` | **8** `docs/requisitos` + **3** `docs/capitulos` (historias de usuario) |
...
- Conteos globales: `CONTRIBUTORS.md` (727/329/291) y
  ...
  `--all`) citaban revs distintos; los vigentes a `fcda4808` son
  **763/343/317** (este archivo). Re-sincronizar al cierre con los
