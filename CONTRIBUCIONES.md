...
Rev de referencia: `9f3702705eab68971b9d275b59da36da57a0ce06`
(fix CRLF k6 + REPORT/.gitignore, rama `codex/asegurar-8-examen`).
Este archivo se commitea en el commit siguiente, que suma +1 a
Panamá; re-sincronizar al cierre con los comandos de abajo.
...
git rev-list --count --no-merges <rev>                            # 1432
git log --no-merges --author=<icajasi|mloorm14|mpanamam> --format=%h -- <ruta>  # por área
```

Totales globales a `9f370270`: **Cajas 763, Loor 343, Panamá 324**
...
| Rol CRediT | Cajas (763) | Loor (343) | Panamá (324) |
|---|---|---|---|
...
| Data curation | **10** `mediciones/sec` (índice, commit `4b7f50a`) + **5** `mediciones/sus` (esqueleto + script, `1ad718b`) | **21** `mediciones/sec` + **8** `mediciones/perf` + `DATA-DICTIONARY.md`, `DATA-PROVENANCE.md` | **2** `mediciones/sus` (retiro declarado N=0, OBS-08) + **3** `mediciones/perf` (REPORT.md serie vigente + k6-run*.json versionados + política documentada) + `ETHICS.md`, `DATA-DICTIONARY.md` (sección SUS, 22 campos) |
| Formal analysis | — | **5** `k6/`+`perf-analysis.py` (Wilcoxon pareado + Cliff's delta) + **8** `mediciones/perf` | `scripts/sus-analysis.ipynb` (reescritura: descriptiva, IC95%, desglose por ítem) + **7** `scripts` |
...
| Writing – review & editing | **12** `CITATION.cff` + **11** `docs/trazabilidad` (unificación cifras, refs rotas) | **57** `docs/capitulos` + **6** `bibliografia.bib` (Peffers, Hevner) + **15** `CITATION.cff` | **8** `docs/requisitos` + **5** `docs/capitulos` (historias de usuario) |
...
- Conteos globales: `CONTRIBUTORS.md` (727/329/291) y
  ...
  `--all`) citaban revs distintos; los vigentes a `9f370270` son
